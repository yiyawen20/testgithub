...
function handlderMsg(msg) {
  ...
  switch (sessType) {
    case webim.SESSION_TYPE.C2C: //私聊消息
      ...
      getApp().msgCallback(contentHtml, fromAccount);
      // switch (subType) {
...
//发送消息(普通消息)
function onSendMsg(tid, msg, callback) {
  selToID = tid;
  if (!selToID) {
...
function getHistoryMsgCallback(msgList, prepage) {
  ...
  for (var j in msgList) {//遍历新消息
    ...
    if (msg.getSession().id() == selToID) {//为当前聊天对象的消息
      ...
      //在聊天窗体中新增一条消息
       //addMsg(msg, prepage);
       console.log('TODO 需要修改 显示历史聊天消息, ', msg)

    }
    lastMsgs[msg.getSession().id()] = msg;
  }
  if (!prepage) {
...
function getLastC2CHistoryMsgs(cbOk, cbError) {
  if (selType == webim.SESSION_TYPE.GROUP) {
    // addSysMsg('当前的聊天类型为群聊天，不能进行拉取好友历史消息操作');
    return;
  }
  ...
  var msgKey = '';
  var options = {
    'Peer_Account': selToID+'', //好友帐号,必须为字符串
    'MaxCnt': reqMsgCount, //拉取消息条数，最大值为15，
...
//向上翻页，获取更早的好友历史消息
function getPrePageC2CHistoryMsgs(cbOk, cbError) {
  if (selType == webim.SESSION_TYPE.GROUP) {
    webim.Log.warn('当前的聊天类型为群聊天，不能进行拉取好友历史消息操作');
    return;
  }
  ...
  if (tempInfo) {
    ...
    webim.Log.warn('获取下一次拉取的c2c消息时间和消息Key为空');
    return;
  }
  var options = {
